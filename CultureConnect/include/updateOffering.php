<?php
    ...
    if (isset($_POST["submit"])){
        ...
        $business = "2"; //TODO: Update when session variables in place
        ...
        if (isset($_GET["of_id_pk"])){ //if offering id exists update the entry that is already there
            ...
            $stmt = $connection->prepare("UPDATE offering SET bus_id_pk = ?, of_category = ?, loc_id_pk = ?, of_name = ?, of_descr = ?, of_details = ?, of_cultural_benefits = ?, of_price_range = ?  
                                            WHERE of_id_pk = ?");
            ...
            $stmt = $connection->prepare("INSERT INTO offering(bus_id_pk, of_category, loc_id_pk, of_name, of_descr, of_details, of_cultural_benefits, of_price_range) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->affected_rows > 0) { //TODO - update to include 
                header('Location: /cultureconnect/04ManageOfferings.php', TRUE, 303);
                exit;
            } else {
